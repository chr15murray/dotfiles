...
FONT=""
MAX_PING=1000
SITES=(8.8.8.8 google.com github.com) #Google DNS; SITES=(google.com youtube.com wikipedia.org github.com) using only one site is recommended for graph consistency
GRAPHMINUTES=30
SITE_INDEX=0
PING_TIMES=
bpp=4
...
FILE_OUT="$HOME/bitbar/temp/ping-$(date +%Y%m%d).txt"
...
makegraph() {
    ...
    readings=$(tail -$((GRAPHMINUTES*6)) "$FILE_OUT" | grep ${SITES[$SITE_INDEX]} | cut -f3 -d ','  | sed -e ':a' -e 'N' -e '$!ba' -e 's/\n/ /g')
    heights=($readings)
